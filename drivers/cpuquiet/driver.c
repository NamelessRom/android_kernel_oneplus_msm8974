...
struct cpuquiet_cpu_stat *cpuquiet_stats;
...
static void cpuquiet_stats_update(struct cpuquiet_cpu_stat *stat, bool up)
{
...
int cpuquiet_quiesence_cpu(unsigned int cpunumber)
{
	...
	if (!err)
		cpuquiet_stats_update(cpuquiet_stats + cpunumber, 0);
...
int cpuquiet_wake_cpu(unsigned int cpunumber)
{
	...
	if (!err)
		cpuquiet_stats_update(cpuquiet_stats + cpunumber, 1);

	return err;
}
EXPORT_SYMBOL(cpuquiet_wake_cpu);

static ssize_t cpuquiet_stats_sysfs_show(struct kobject *kobj,
			struct attribute *attr, char *buf)
{
	...
	cpuquiet_stats_update(stat, was_up);
...
static const struct sysfs_ops cpuquiet_stats_sysfs_ops = {
	.show = cpuquiet_stats_sysfs_show,
};

static struct kobj_type ktype_cpu_stats = {
	.sysfs_ops = &cpuquiet_stats_sysfs_ops,
...
int cpuquiet_register_driver(struct cpuquiet_driver *drv)
{
	...
	cpuquiet_stats = kzalloc(nr_cpu_ids * sizeof(*cpuquiet_stats), GFP_KERNEL);
	if (!cpuquiet_stats)
		return -ENOMEM;

	for_each_possible_cpu(cpu) {
		cur_jiffies = get_jiffies_64();
		cpuquiet_stats[cpu].last_update = cur_jiffies;
		if (cpu_online(cpu))
			cpuquiet_stats[cpu].up_down_count = 1;
		cpu_dev = get_cpu_device(cpu);
		if (cpu_dev) {
			cpuquiet_add_dev(cpu_dev, cpu);
			cpuquiet_cpu_kobject_init(&cpuquiet_stats[cpu].cpu_kobject,
					&ktype_cpu_stats, "cpuquiet_stats", cpu);
		}
	}
...
void cpuquiet_unregister_driver(struct cpuquiet_driver *drv)
{
	...
	for_each_possible_cpu(cpu) {
		kobject_put(&cpuquiet_stats[cpu].cpu_kobject);
